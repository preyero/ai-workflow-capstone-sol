...
from scripts.model import model_train, model_load, model_predict
from scripts.model import MODEL_VERSION, MODEL_VERSION_NOTE
...
def predict():
    ...
    if not ('country' in query):
        print("ERROR API (predict): missing parameter 'country'")
        return jsonify([])
    if not ('year' in query):
        print("ERROR API (predict): missing parameter 'year'")
        return jsonify([])
    if not ('month' in query):
        print("ERROR API (predict): missing parameter 'month'")
        return jsonify([])
    if not ('day' in query):
        print("ERROR API (predict): missing parameter 'day'")
        return jsonify([])
    ...
    _result = model_predict(query['country'],
...
def train():
    ...
    model = model_train(data_dir=None, smodel='rf',test=test)
