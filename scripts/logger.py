...
def update_train_log(tag, smodel,is_test,data_shape,eval_test,runtime,MODEL_VERSION,MODEL_VERSION_NOTE,test=False):
    """
    ...
    if test:
        ...
        logfile = os.path.join("logs","train-{}-{}-{}.log".format(tag,today.year, today.month))
        
    ## write the data to a csv file    
    header = ['unique_id','timestamp', 'tag','smodel','test_data','x_shape','eval_test','model_version',
    ...
    with open(logfile,'a') as csvfile:
        ...
        to_write = map(str,[uuid.uuid4(),time.time(),tag, smodel, is_test, data_shape,eval_test,
...
def update_predict_log(tag, y_pred,y_proba,query,runtime,MODEL_VERSION,test=False):
    ...
    today = date.today()
    if test:
        logfile = os.path.join("logs","predict-test-{}.log".format(tag))
    else:
    ...
    with open(logfile,'a') as csvfile:
        ...
        to_write = map(str,[uuid.uuid4(),time.time(),tag,y_pred,y_proba,query,
